...
# all the functions in source files in current dir
# and subdirs.
#
# V2 - Enhanced version to work with various formattings
# of source code. Take advantage of indent C formatter
# preceeded by comment stripping. Should work on most
# C code layouts...
#
#  V. Santos, Dez-2014, vitor@ua.pt
#
# V3 - New version with extern "C" declarations for C++ compilers
# and sepration of what are C++ and C files based on file
# extension (NB. This may fail if conventions are broken)
#
#  V. Santos, Dez-2015, vitor@ua.pt
#
# V4 - Another improvement to include namespaces in C++ definitions
# (This one was tricky because ctags has limited ways of expressing namespaces
#
#  V. Santos, 27-Set-2017, vitor@ua.pt
# 
# V5 - minor adjustmente to remove empty lines in intermediate functions
#
####################################################

if [ "$#" -ge 1 ]; then
	echo "genproto.sh - Script to generate a file with C/C++ function prototypes."
	echo "V5, Sep 2017, v. santos, UA."
    exit
fi

command -v ctags  >/dev/null 2>&1 || { echo "ctags is not installed or not in path. sudo apt install ctags" >&2; return; }
command -v indent >/dev/null 2>&1 || { echo "indent is not installed or not in path. sudo apt install indent" >&2; return; }
